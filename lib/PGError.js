...
module.exports = class PGError extends ExtendableError {
   ...
   * @param {any} config
   */
  constructor(err, query, config) {
    query = formatQuery(query);
    
    const message = createMessage(err, query, config);
    super(message);
 ...
 * @param {any} config
 */
function createMessage(err, query, config) {
  const verbosityLevel   = config && typeof config.verbosityLevel   === 'number'? config.verbosityLevel   : pgConst.PQERRORS_VERBOSE;
  const showContextLevel = config && typeof config.showContextLevel === 'number'? config.showContextLevel : pgConst.PQSHOW_CONTEXT_NEVER;
  const hideQuery        = config && config.hideQuery;
  const hideQueryValues  = config && config.hideQueryValues;
